...
var initWorkflow = (config) => {
  ...
  const prizesDistributedEventHash = keccak256(toBytes("PrizesDistributed(uint256,address[],uint256[])"));
...
var onPrizesDistributed = async (runtime2, log) => {
  try {
    ...
    runtime2.log(`Prizes Distributed - Session ID: ${sessionId}, Winners: ${winners.length}, Total Prize Pool: ${prizeAmounts.reduce((sum, amount) => sum + amount, 0n)}`);
    for (let i2 = 0;i2 < winners.length; i2++) {
      runtime2.log(`  Winner ${i2 + 1}: ${winners[i2]} - Prize: ${prizeAmounts[i2]} USDC`);
    }
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    runtime2.log(`Error processing PrizesDistributed event: ${errorMessage}`);
  }
};
async function main() {
